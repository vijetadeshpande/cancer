...
%Using below values added by Prashant on 10 July 2016 
     constantDwellTimes(1,i)= 5.22;
     constantDwellTimes(2,i)= 3.045 ;
     constantDwellTimes(3,i)= 2.32;
     constantDwellTimes(4,i)= 2;
...
cDwellTimes1 = interp1(AgeArrayMid,constantDwellTimes(1,:),[0:100],'cubic');
% DwellTimes1(80:end)=DwellTimes1(80);

cDwellTimes2 = interp1(AgeArrayMid,constantDwellTimes(2,:),[0:100],'cubic');
% DwellTimes2(80:end)=DwellTimes2(80);

cDwellTimes3 = interp1(AgeArrayMid,constantDwellTimes(3,:),[0:100],'cubic');
% DwellTimes3(80:end)=DwellTimes3(80);

cDwellTimes4 = interp1(AgeArrayMid,constantDwellTimes(4,:),[0:100],'cubic');
% DwellTimes4(80:end)=DwellTimes4(80);

constantDwellTimesF=[cDwellTimes1;cDwellTimes2;cDwellTimes3;cDwellTimes4];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[Mu_Tx, ImpactAge, BaseMortality, TxCoverage, StgDist, RelativeMortTx, RelativeMortNoTx] = Mortality(DwellTimesF, MortDFbyAge, PrevalenceF, StageDist, smooth(PAR{5},'lowess'),PAR);
...
for J=1:length(AgeArrayLower)
    ...
    MortDFageGroup(J)=dot(MortDFbyAge(indA),probAgeA(indA))/sum(probAgeA(indA));
...
MortDFageGroup(end)=MortDFageGroup(end-1);                                                                                                              
MortC(end)=MortC(end-1);




%TimeToDiagnosis (taking half the time in last stage 
...
for i = 1 : size(DwellTimes,2)
    ...
    for j = 1:nsims
       ...
       sumTime(2) = sumTime(2) + Insitu + 0.5*Local;% half time in last stage 
...
for i = 1 : size(DwellTimes,2)
   ModTimeToDiagnosis(1,i) = (1 - CumStageDISTwithDCO (1))/((1/DwellTimes(1,i) + MortDFageGroup(1,i)) * CumStageDISTwithDCO(1));
   ModTimeToDiagnosis(2,i) = (1 - CumStageDISTwithDCO (2))/((1/(DwellTimes(1,i) + DwellTimes(2,i)) + MortDFageGroup(1,i))* CumStageDISTwithDCO(2));
   ModTimeToDiagnosis(3,i) = (1 - CumStageDISTwithDCO (3))/((1/(DwellTimes(1,i) + DwellTimes(2,i) + DwellTimes(3,i))+ MortDFageGroup(1,i))* CumStageDISTwithDCO(3));
   ModTimeToDiagnosis(4,i) = (1 - CumStageDISTwithDCO (4))/((1/(DwellTimes(1,i) + DwellTimes(2,i) + DwellTimes(3,i) + DwellTimes(4,i))+ MortDFageGroup(1,i))* CumStageDISTwithDCO(4));
end 
% for i = 1 : size(DwellTimes,2)
...
Incidence = IncidenceFunc(probAgeA,MortDFbyAge,prevalenceActual,cancerMortalityArray, AgeArrayLower,AgeArrayUpper);   
...
OnsetRates = PrevStageRates (Incidence,TimeToDiagnosis,StageDist / (1+DCO) ,probAgeA,MortDFbyAge,prevalenceActual,cancerMortalityArray, AgeArrayLower,AgeArrayUpper);
...
RES{8}=MortDFageGroup';% Disease free mortality
...
%%FIT ALL AGES TOGETHER
% [beta,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(@(x,X)...
% PopSim(x,X,PAR,Incidence,OnsetRatesF,DwellTimesF,AgeArrayLower,AgeArrayUpper,Mu_Tx, BaseMortality, 0, 1000000),...
...
for t=1:t_max
    ...
    % Deaths in preclinical
    death_pre_clinical_4(:,1) = death_pre_clinical_4(:,1) + POP(:,5).*(mu+lambda(:,4))*dt;
...
for J=1:length(AgeArrayLower)
    ...
    death_rate_distant(J) = sum(death_distant_age_group(:)/sum(p_a(:)));
...
% 
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     % Editing from this point is done by Vijeta D
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     
% %%                    
...
%     end
%     DCO = 0.1;
%     lamdaDCO=zeros(1,size(AgeArrayLower,2));
%     for n=(1:12)
%            lamdaDCO(1,n)=lamda(4,n) *(1-DCO)/DCO;
%            
%     end
...
%             end
%             
% %                 sumIloopNumerator = sumIloopNumerator + (  (1- exp(-t*lamdaDCO(1,k))) - (1- exp(-(t-1)*lamdaDCO(1,k))) );
%             
%            
%             muProduct=1;
%             for j = k:a
...
%         end
%         
% %                 sumIloopDenominator = sumIloopDenominator +  (1- exp(-t*lamdaDCO(1,a)));
%        
...
%             end
%             %sumIloopNumerator = sumIloopNumerator +  exp(-t*lamdaDCO(1,k)) ;
%             
%             muProduct=1;
%             for j = k:a
%              muProduct = muProduct * exp(-(mid_age_group(j)-mid_age_group(j-1)) *mu(1,j));
